...
+function (e) {
    ...
    for (var i = 0; i < item_list.length; i++) {
        item_list[i].addEventListener("click", function(ev) {
            ...
            var id_conditioning = this.getAttribute("data-conditioning-type-id");
            var inCard = false;
            for(var j = 0 ; j < panier.length ; j++)
            {
                if(panier[j].conditioningTypeId == id_conditioning) {
                    inCard = true;
                    break;
                }
            }
    ...
    function addItem(id, cost) {
        ...
        document.getElementById("items").innerHTML += "<div class='cart-item' id='item" + cindex +
            ...
            "<span class='cart cart-item-name'>" + translate.quantity + "<input class='quantityInput' type='number' min='1' max='1023' data-id='" + id +"' data-conditioning-type-id='" + curConditioningTypeId +"' value='"+quantity+"'></span></div>";
        document.getElementById("items-counter").innerHTML += "<span class='animate '><div class='caddie glyphicon glyphicon-shopping-cart'></div>" + curCounter +
            "<span class='circle'></span></span>";
